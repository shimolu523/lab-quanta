...
class logic_circuit:
    ...
    def register(self):
        ...
        self.table = helpers.typical_io_table([ ("X Set", 'o'), ("Y1 Read", 'i'), ("Y2 Read", 'i'), ("Y3 Read", 'i'), ("Y4 Read", 'i')])
        ...
        self.tree.add_row("Y3 Scaling", 1.0)
        self.tree.add_row("Y4 Scaling", 1.0)
        self.tree.add_row("Pause", .05)
        self.tree.add_row("Finish at", 0.0)

        self.tree.restore_state("TimeDepScan4")
        self.table.restore_state("TimeDepScan4")
    ...
    def unregister(self):
        self.tree.save_state("TimeDepScan4")
        self.table.save_state("TimeDepScan4")
    ...
    def initialize(self, filename, dirname):
        ...
        y3key = self.table.get_choice("Y3 Read", 'i')
        if (y3key != "<empty>"):
            Y3_read_function = self.open_io_read_streams[y3key]
        else:
            Y3_read_function = None 

        y4key = self.table.get_choice("Y4 Read", 'i')
        if (y4key != "<empty>"):
            Y4_read_function = self.open_io_read_streams[y4key]
        else:
            Y4_read_function = None 
        ...
        run_desc = {'f_Y1read' : Y1_read_function, 
                    ...
                    'f_Y3read' : Y3_read_function,
                    'y3label' : '%s * %.2g'%(y3key, self.tree.get_data('Y3 Scaling', 1)),
                    'f_Y4read' : Y4_read_function,
                    'y4label' : '%s * %.2g'%(y4key, self.tree.get_data('Y4 Scaling', 1)),
                    'f_Xset' : set_function,
                    'xlabel' : 'time [s]',
                    ...
                    'y3scale' : self.tree.get_data('Y3 Scaling', 1),
                    'y4scale' : self.tree.get_data('Y4 Scaling', 1),
    ...
    def run(self):
        worker.feed('''
try:
     latestdata = kernel_TimeDepScan4(run_desc, gui_exports)
finally:
