# CavityCoolRabi_0deg_Doppler.py
# Forked by Hans on 2011-06-27 to do Cavity Cooling in 0 degree set up from Doppler limit.
# Changes in beams are made to reflect this and SCloops have been changed to 0 in DataRun.
ARGS = ['StartT', 'EndT', 'CavityOnDF', 'ReadoutDF', 'ScanPoints', 'ScanRepeat', 'LockOption', 'CavLockOn', 'ProbeLockOn', 'ProbeSet','ProbeSet_SB', 'IonLockOn', 'IonSet','IonPosition']

import Stabilization
import CavityStabilization

def RunScript(filename, dirname, StartT, EndT, CavityOnDF, ReadoutDF, ScanPoints, ScanRepeat, LockOption, CavLockOn, ProbeLockOn, ProbeSet,ProbeSet_SB, IonLockOn, IonSet, IonPosition):
...
def CavityCoolRabi_0deg_IONPOS(filename, dirname, StartT, EndT, CavityOnDF, ReadoutDF, ScanPoints, ScanRepeat, LockOption, CavLockOn, ProbeLockOn, ProbeSet, ProbeSet_SB,IonLockOn, IonSet,IonPosition):
...
#           plotLabel = ['BlueCav', 'RedCenter', 'RedPiTime', 'RedCenter'][LockOption]
        ...
        for i in range(int(ScanRepeat)):
            ...
            while j <= int(ScanPoints):
                ...
                print "IonPosSet(IonPos+IonPosition)=IonPosSet(%f+%f)"%(IonPos,IonPosition)
...
#                time.sleep(2) # Tried this without any remarkable results.
        ...
        print time.strftime('%H:%M:%S'),__file__,"Exception occurred in RunScript(CavityCoolRabi_0deg_Doppler.py):", e
        traceback.print_exc()
